...
public class AbstractTest {
	...
	public float pack(int binWidth, int binHeight, int[] vals, MaxRectsContainer.FreeRectChoiceHeuristic heuristic, boolean output) {
		
		// Create a bin to pack to, use the bin size from command line.
		if (output) 
			System.out.printf("Initializing bin to size %dx%d.\n", binWidth, binHeight);
		MaxRectsContainer bin = new MaxRectsContainer(binWidth, binHeight);
